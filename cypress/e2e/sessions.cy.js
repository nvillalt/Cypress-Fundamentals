...
describe("Sessions page", () => {
    ...
    beforeEach(() => {
        ...
        // Define aliases here. We use .as to define an alias. To call an alias > @AliasName
    ...
    it("should navigate to conference sessions page and view day filter buttons", () => {
        // Validate that buttons to filter by day exist
            // Assertions that can be used in cypress: https://docs.cypress.io/guides/references/assertions 
            // Bundled: Chai assertion library, Sinon and jQuery assertions. Example below:
    ...
    it("should filter sessions and only display Wednesday sessions when the button is clicked", () => {
        // Commented out due to the imposibility of make this work
/*
        cy.intercept("POST", "http://localhost:4000/graphql").as("getSessionInfo");
        cy.get("@WednesdayBtn").click();
        cy.wait("@getSessionInfo"); */

        // Looks for the data-cy day, checks each day and makes sure that, after clicking "Thursday", only that is visible
    ...
    it("should filter sessions and only display Thursday sessions when the button is clicked", () => {
    ...
    it("should filter sessions and only display Friday sessions when the button is clicked", () => {
        ...
        // Looks for the data-cy day, checks each day and makes sure that, after clicking "Friday", only that is visible
        cy.get("[data-cy=day]").contains("Wednesday").should("not.exist");
        cy.get("[data-cy=day]").contains("Thursday").should("not.exist");
        cy.get("[data-cy=day]").contains("Friday").should("be.visible");
    })

    it("should filter sessions and displays All Sessions when the button is clicked", () => {
        
        cy.intercept("POST", "http://localhost:4000/graphql").as("getSessionInfo");
        cy.get("@AllSessionsBtn").click();
        cy.wait("@getSessionInfo");

        // Looks for the data-cy day, checks each day and makes sure that, after clicking "All Sessions", are visible
        cy.get("[data-cy=day]").contains("Wednesday").should("be.visible");
        cy.get("[data-cy=day]").contains("Thursday").should("be.visible");
        cy.get("[data-cy=day]").contains("Friday").should("be.visible");
    })
